...
import users from '../lib/generated-users.json'; // Assuming this is the user data
...
export default function Home() {
        ...
        return (
          <div className="max-w-md mx-auto bg-white dark:bg-black min-h-screen flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm">
                See your likes and interactions
              </p>
            </div>
          </div>
        );
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-gray-50 dark:bg-black">
      {showVideo && (
        <div className="fixed top-0 left-1/2 -translate-x-1/2 h-[90vh] w-screen mx-auto z-50">
          <button className="absolute top-4 right-4 bg-black bg-opacity-50 text-lucida rounded-full p-2 z-50">
            ✖
          </button>
          <VideoPlayer
            src={users?.find((u) => u.displayName === 'Yann')?.video || ''}
      ...
      <TopNav />
      <div className="pt-16 pb-20">{renderContent()}</div>
